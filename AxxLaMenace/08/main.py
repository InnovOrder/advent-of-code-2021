...
# PART 1
def solve_first_puzzle(inputsAndOutputs):
    outputs = [elem[1] for elem in inputsAndOutputs]
    count = sum(sum([1 for elem in output.split() if len(elem) in [2,3,4,7]]) for output in outputs)
    return count
...
if __name__ == '__main__':
    ...
    inputsAndOutputs = [lines[i].split(' | ') for i in range(len(lines))]
    print("result first puzzle:", solve_first_puzzle(inputsAndOutputs))
